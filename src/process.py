"""
@author: wrench <ljl260435988@gmail.com>
"""
...
def local_train(index, opt, global_model, global_icm, optimizer, save=False):
    ...
    env, num_states, num_actions = create_train_env(index+1,7)
    ...
    while True:
        ...
        for _ in range(opt.num_local_steps):
            ...
            #print(action)
            actions = m.sample().cpu().numpy().squeeze()
            #print(actions)
           # ee = torch.tensor([action]).cuda()
           # ef =  torch.tensor([action])
            next_state, reward, round_done, stage_done, game_done = env.step(int(actions))
            ...
            if round_done or stage_done or game_done:
                curr_step = 0
                next_state = torch.from_numpy(env.reset(round_done,stage_done,game_done)) #重置游戏
                if opt.use_gpu:
                    next_state = next_state.cuda()
